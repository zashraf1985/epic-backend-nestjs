...
export class EpicAuthController {
    ...
    @Get('launch-callback')
    @Redirect()
    async launchCallback(@Query() queryParams: Record<string, any>): Promise<any> {
        ...
        const metadata = await this.smartOnFhirAuthService.getMetadata(queryParams["iss"]);
        if (!metadata) {
            return new HttpException("Error fetching metadata", 500)
        }

        return { url: await this.smartOnFhirAuthService.buildAuthUrl(metadata, queryParams["launch"], queryParams["iss"]) };
    }
